...
export async function createApp({
  ...
  console.log(`Creating a new model0 SDK app in ${green(appPath)}.`)
  console.log()

  // Download example from GitHub repository
  const repoUrl = 'https://github.com/neopilot-ai/model0-sdk'
      ...
      for (const [name, version] of Object.entries(deps)) {
        if (version === 'workspace:*') {
          ...
          const packageVersions: Record<string, string> = {
            'model0-sdk': '^0.16.4',
            '@model0-sdk/react': '^0.5.0',
            '@model0-sdk/ai-tools': '^0.3.8',
          }
